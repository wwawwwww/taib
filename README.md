...
После запуска `tgauto` ты увидишь шесть основных действий.
...
Режим для общения в Telegram-группе. Есть два сценария:

- `Live`: бот добавлен в чат, копит сообщения выбранного участника до лимита `GROUP_LIVE_MIN_MESSAGES`, потом отвечает другим участникам в его манере.
- `Archive`: ты загружаешь архив чата, выбираешь участника из списка авторов, бот имитирует его стиль и отвечает в выбранном рабочем чате.

Внутри раздела можно:

- выбрать сценарий `Live` или `Archive`;
- выбрать рабочий групповой чат из Telegram updates без ручного `curl`;
- в `Live` выбрать участника из видимых сообщений и задать, сколько его сообщений нужно накопить;
- в `Archive` загрузить архив сообщений группы и выбрать участника из найденного списка авторов;
- настроить шанс ответа `GROUP_REPLY_PROBABILITY`;
- настроить минимальную паузу между ответами.
- на macOS при включении режима ставится фоновый launchd-агент `com.codex.group-chat`, который периодически запускает `group-chat`.

Важно: чтобы бот видел обычные сообщения группы, у BotFather может понадобиться выключить privacy mode для бота. Иначе Telegram будет присылать ему только команды, ответы на его сообщения и упоминания.

Для старого архива группы:

1. Экспортируй чат из Telegram Desktop в `.json` или `.html`.
2. В TUI открой `Групповой чат`.
3. Выбери `Загрузить архив и выбрать участника`.
4. Укажи путь к файлу или папке экспорта.
5. TG Auto покажет участников, которые хотя бы раз писали в архиве; выбери нужного человека стрелками.

После выбора профиль стиля сразу строится по сообщениям этого участника и используется для будущих ответов в выбранном рабочем чате. Архив и рабочий чат могут быть разными.
...
- отключает `AUTOPILOT_ENABLED`, `SPONTANEOUS_ENABLED` и `GROUP_CHAT_ENABLED`;
- снимает фоновые launchd-агенты на macOS;
- завершает текущие процессы публикации `daily_poster`.

## Важное правило режимов

`Автогенерация`, `Отслеживание` и `Групповой чат` взаимоисключающие.
...
python -m daily_poster group-chat
python -m daily_poster group-import-archive path/to/group-export
python -m daily_poster group-use-participant 1
```
