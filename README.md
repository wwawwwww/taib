...
Режим для общения в Telegram-группе:

- бот читает новые сообщения группы через Telegram updates;
- учится на сообщениях выбранного участника;
- строит профиль его стиля: длина реплик, пунктуация, регистр, сленг, характерные слова;
- с заданной вероятностью отвечает на сообщения других участников;
- хранит локальную память группы в `data/group_memory.json`.

Внутри раздела можно:

- включить режим `group`;
- указать группу;
- указать участника-образец по `user_id`, `@username` или имени;
- загрузить архив сообщений группы и выбрать участника из найденного списка авторов;
...
5. TG Auto покажет участников, которые хотя бы раз писали в архиве; выбери номер нужного человека.

После выбора профиль стиля сразу строится по сообщениям этого участника и используется для будущих ответов в группе.
